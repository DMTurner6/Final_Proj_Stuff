...
void lcdSend4bit(unsigned char data)
{
    LATAbits.LATA4 = data & 0x01;
    LATAbits.LATA5 = (data >> 1) & 0x01;
    LATAbits.LATA6 = (data >> 2) & 0x01;
    LATAbits.LATA7 = (data >> 3) & 0x01;
...
void lcdSetCursor (unsigned char col, unsigned char row)
{
  if (row >= 2)
    row = 1;
  unsigned char _rowOffset[2] = {0x00, 0x40};
  lcdCmd (0x80 | (col + _rowOffset[row]));
}

void lcdDispOn(void)
{
  _dispCon |= 0x04;
  lcdCmd(0x08 | _dispCon);
}
...
void lcdInit(void)
{
    ...
    _dispFunc = 0x00 | 0x08 | 0x00;
    lcdCmd(0x20 | _dispFunc);

    _dispCon = 0x04 | 0x00 | 0x00;
    lcdCmd(0x08 | _dispCon);
